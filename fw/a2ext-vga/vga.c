...
static void vga_data_setup(PIO pio, uint sm) {
    ...
    for(int i = 0; i < 9; i++) {
        ...
        gpio_set_drive_strength(CONFIG_PIN_RGB_BASE + i, GPIO_DRIVE_STRENGTH_8MA);
    }
...
void vga_submit_scanline(struct vga_scanline *scanline) {
    ...
    /* Upstream RGB333 uses R in bits 6..8; carrier has R in bits 0..2.
       Preserve the PIO jump token in each 16-bit halfword. */
    for (unsigned i=0; i<scanline->length; ++i) {
        uint32_t v = scanline->data[i];
        scanline->data[i] = (v & ~0x01c701c7u) |
            ((v & 0x00070007u) << 6) | ((v & 0x01c001c0u) >> 6);
    }
